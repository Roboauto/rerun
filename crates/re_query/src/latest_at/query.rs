...
use re_types_core::{components::ClearIsRecursive, ComponentName, Loggable, SizeBytes};
...
) -> Option<(RowId, TimeInt, Box<dyn Array>)> {
    store
        .latest_at(query, entity_path, component_name)
        .into_iter()
        .filter_map(|chunk| chunk.latest_at(query, component_name))
...
impl LatestAtCache {
    ...
    pub fn latest_at(
        ...
        if let Some((row_id, data_time, array)) =
