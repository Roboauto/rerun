...
/// A [`StoreSubscriber`] subscribes to atomic changes from all [`DataStore2`]s through [`StoreEvent2`]s.
///
...
mod tests {
    ...
    use crate::{DataStore2, GarbageCollectionOptions, StoreSubscriber};

    use super::*;

    /// A simple [`StoreSubscriber`] for test purposes that just accumulates [`StoreEvent2`]s.
    #[derive(Debug)]
    struct AllEvents {
